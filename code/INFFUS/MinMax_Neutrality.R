...
# Minimal Neutral Set
DataSet$MinNeuSet <- as.factor(ifelse(DataSet$BingSentiment == "neutral" & DataSet$CoreNLPSentiment == "neutral"
                            & DataSet$MCSentiment == "neutral" & DataSet$MicrosoftSentiment == "neutral" & DataSet$BingSentiment == "neutral"
                            & DataSet$SentiStrSentiment == "neutral" & DataSet$VADERSentiment == "neutral", "1", "0"))
DataSet$MaxNeuSet <- as.factor(ifelse(DataSet$BingSentiment == "neutral" | DataSet$CoreNLPSentiment == "neutral"
                                      | DataSet$MCSentiment == "neutral" | DataSet$MicrosoftSentiment == "neutral" | DataSet$BingSentiment == "neutral"
                                      | DataSet$SentiStrSentiment == "neutral" | DataSet$VADERSentiment == "neutral", "1", "0"))
...
weightAbs <- function(x){abs(x-0.5)}

## Compute OWA weights
for(i in 1:nrow(DataSet)){
  # print(i)
  weigthAbs_aux <- weightAbs(DataSet[i,3:8])
  indMax <- which(weigthAbs_aux == max(weigthAbs_aux))[1]
  OWASentiment <- c(OWASentiment, DataSet[i,indMax+2])
}

DataSet$MinNeutr <- OWASentiment

DataSet$MinNeutrSentiment <- ifelse(DataSet$MinNeutr >= 0.6, "positive", ifelse(DataSet$MinNeutr <= 0.4, "negative", "neutral"))
DataSet <- DataSet[, c(1, 2, 22)]
DataSet$MinNeutrSentiment <- factor(DataSet$MinNeutrSentiment, levels = c("positive", "neutral", "negative"))
...
for(i in 1:totCorpus){
  ...
  setnames(DataSetAux_DTM, "DataSetAux[DataSetAux[, 3] != \"neutral\", 3]", "MinNeutrSentiment")
  ...
  # Building the model
  setnames(DataSetAux_DTM, "MinNeutrSentiment", "SentimentClass")
  ...
  for(k in 1:length(Models)){
    ...
    # Print Results
    pathResults <- paste0("./results/MinNeutrality/", levels(DataSet$Corpus)[i], "_", Models[k],".txt")
    sink(pathResults)
    
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    print(paste0("EXPERIMENT: ", levels(DataSet$Corpus)[i], " ", "MinNeutrality", " ", Models[k]))
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
