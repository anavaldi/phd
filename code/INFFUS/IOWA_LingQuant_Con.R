...
table(DataSet$Corpus, DataSet$AtLeastSentiment)
...
for(i in 1:totCorpus){
  for(j in 1:3){
    ...
    for(k in 1:length(Models)){
      ...
      # Print Results
      pathResults <- paste0("./results/QuanLingConNeutrality/", levels(DataSet$Corpus)[i], "_", colnames(DataSet)[ncol(DataSet)-3+j], "_", Models[k],".txt")
      sink(pathResults)
