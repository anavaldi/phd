...
for(i in 1:nrow(DataSet)){
  ...
  DataSet$ConNeuSentiment[i] <- sum((weigthAbs_aux/sum(weigthAbs_aux))*DataSet[i, 3:8])
}

DataSet$ProNeuSet <- ifelse(DataSet$ConNeuSentiment > 0.4 & DataSet$ConNeuSentiment < 0.6, 1, 0)
aggregate(DataSet$ProNeuSet ~ DataSet$Corpus, FUN=sum)

DataSet$ConNeuSentimentName <- ifelse(DataSet$ConNeuSentiment >= 0.6, "positive", ifelse(DataSet$ConNeuSentiment <= 0.4, "negative", "neutral"))
DataSet <- DataSet[, c(1, 2, 15, 17)]
DataSet$ConNeuSentimentName <- factor(DataSet$ConNeuSentimentName, levels = c("positive", "neutral", "negative"))
...
for(i in 1:totCorpus){
  ...
  for(k in 1:length(Models)){
    ...
    # Print Results
    pathResults <- paste0("./results/ConNeutrality/", levels(DataSet$Corpus)[i], "_", Models[k],".txt")
    sink(pathResults)
    
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    print(paste0("EXPERIMENT: ", levels(DataSet$Corpus)[i], " ", "ConNeutrality", " ", Models[k]))
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
