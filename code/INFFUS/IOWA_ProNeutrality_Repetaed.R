...
## Compute OWA weights
DataSet_Weights$BingWeights <- NA
DataSet_Weights$CoreNLPWeights <- NA
DataSet_Weights$MCScaleWeights <- NA
DataSet_Weights$MicrosoftWeights <- NA
DataSet_Weights$SentiStrWeights <- NA
DataSet_Weights$VADERWeights <- NA

for(i in 1:nrow(DataSet)){
  weigthAbs_aux <- weightAbs(DataSet[i, 3:8])
  DataSet_Weights$BingWeights[i] <- weightAbs(DataSet[i, 3])
  DataSet_Weights$CoreNLPWeights[i] <- weightAbs(DataSet[i, 4])
  DataSet_Weights$MCScaleWeights[i] <- weightAbs(DataSet[i, 5])
  DataSet_Weights$MicrosoftWeights[i] <- weightAbs(DataSet[i, 6])
  DataSet_Weights$SentiStrWeights[i] <- weightAbs(DataSet[i, 7])
  DataSet_Weights$VADERWeights[i] <- weightAbs(DataSet[i, 8])
  
  DataSet$ProNeuSentiment[i] <- sum((weigthAbs_aux/sum(weigthAbs_aux))*DataSet[i, 3:8])
  DataSet_Weights$ProNeuSentiment[i] <- sum((weigthAbs_aux/sum(weigthAbs_aux))*DataSet[i, 3:8])
}

write.csv(DataSet_Weights, paste0("./results/ProNeutrality_Repeated/DataSet_Weights.csv"), row.names = FALSE)
...
for(i in 1:totCorpus){
  ...
  trainIndex <- createDataPartition(DataSetAux_DTM$SentimentClass, times = 1, p = 0.85, list = FALSE)
  ...
  for(k in 1:length(Models)){
    ...
    # Print Results
    pathResults <- paste0("./results/ProNeutrality_Repeated/", levels(DataSet$Corpus)[i], "_", Models[k],".txt")
    sink(pathResults)
