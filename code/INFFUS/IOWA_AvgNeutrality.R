...
DataSet$AvgNeuSentiment <- rowMeans(DataSet[, 3:8])


DataSet$AvgNeuSet <- ifelse(DataSet$AvgNeuSentiment > 0.4 & DataSet$AvgNeuSentiment < 0.6, 1, 0)
aggregate(DataSet$AvgNeuSet ~ DataSet$Corpus, FUN=sum)

DataSet$AvgNeuSentimentName <- ifelse(DataSet$AvgNeuSentiment >= 0.6, "positive", ifelse(DataSet$AvgNeuSentiment <= 0.4, "negative", "neutral"))
DataSet <- DataSet[, c(1, 2, 15, 17)]
DataSet$AvgNeuSentimentName <- factor(DataSet$AvgNeuSentimentName, levels = c("positive", "neutral", "negative"))
...
for(i in 1:totCorpus){
  ...
  for(k in 1:length(Models)){
    ...
    # Print Results
    pathResults <- paste0("./results/AvgNeutrality/", levels(DataSet$Corpus)[i], "_", Models[k],".txt")
    sink(pathResults)
    
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
    print(paste0("EXPERIMENT: ", levels(DataSet$Corpus)[i], " ", "AvgNeutrality", " ", Models[k]))
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
